...
p=np.zeros(nx)
C_x=0.2
S_1=np.zeros(nx)#artificial viscosity
S_2=np.zeros(nx)
S_3=np.zeros(nx)
S_1p=np.zeros(nx)
S_2p=np.zeros(nx)
...
#we need time step value now
		...
		for i in range(1,nx-1):
			S_1[i]=(C_x*abs(((rho[i+1]*T[i+1])-(2*(rho[i]*T[i]))+(rho[i-1]*T[i-1]))))/((rho[i]*T[i])+(2*(rho[i]*T[i]))+(rho[i]*T[i]))*(U_1[i+1]-(2*U_1[i])+U_1[i-1])
			S_2[i]=(C_x*abs(((rho[i+1]*T[i+1])-(2*(rho[i]*T[i]))+(rho[i-1]*T[i-1]))))/((rho[i]*T[i])+(2*(rho[i]*T[i]))+(rho[i]*T[i]))*(U_2[i+1]-(2*U_2[i])+U_2[i-1])
			S_3[i]=(C_x*abs(((rho[i+1]*T[i+1])-(2*(rho[i]*T[i]))+(rho[i-1]*T[i-1]))))/((rho[i]*T[i])+(2*(rho[i]*T[i]))+(rho[i]*T[i]))*(U_3[i+1]-(2*U_3[i])+U_3[i-1])
...
#get predicted values of U
		...
		for i in range(nx-1):
			...
			U_1p[i]=U_1[i]+(dU1_dt_n[i]*dt)+S_1[i]
			U_2p[i]=U_2[i]+(dU2_dt_n[i]*dt)+S_2[i]
			U_3p[i]=U_3[i]+(dU3_dt_n[i]*dt)+S_3[i]
...
#average time derivative
		...
		for i in range(1,nx-1):
			S_1p[i]=(C_x*abs(((rho_p[i+1]*T_p[i+1])-(2*(rho_p[i]*T_p[i]))+(rho_p[i-1]*T_p[i-1]))))/((rho_p[i]*T_p[i])+(2*(rho_p[i]*T_p[i]))+(rho_p[i]*T_p[i]))*(U_1p[i+1]-(2*U_1p[i])+U_1p[i-1])
			S_2p[i]=(C_x*abs(((rho_p[i+1]*T_p[i+1])-(2*(rho_p[i]*T_p[i]))+(rho_p[i-1]*T_p[i-1]))))/((rho_p[i]*T_p[i])+(2*(rho_p[i]*T_p[i]))+(rho_p[i]*T_p[i]))*(U_2p[i+1]-(2*U_2p[i])+U_2p[i-1])
			S_3p[i]=(C_x*abs(((rho_p[i+1]*T_p[i+1])-(2*(rho_p[i]*T_p[i]))+(rho_p[i-1]*T_p[i-1]))))/((rho_p[i]*T_p[i])+(2*(rho_p[i]*T_p[i]))+(rho_p[i]*T_p[i]))*(U_3p[i+1]-(2*U_3p[i])+U_3p[i-1])

#final update to U
		for i in range(1,nx-1):
			U_1n[i]=U_1[i]+(dt*dU1_dt_av[i])+S_1p[i]
			U_2n[i]=U_2[i]+(dt*dU2_dt_av[i])+S_2p[i]
			U_3n[i]=U_3[i]+(dt*dU3_dt_av[i])+S_3p[i]
		U_1n[0]=A[0]
